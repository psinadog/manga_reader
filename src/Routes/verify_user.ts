export class Verify {
    ...
    verify() {
        ...
        if(this.user.email.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/gi) === null) this.verify_user.email = false; 

        if(this.is_empty(this.user)) {
            return this.is_empty(this.user);
        }
